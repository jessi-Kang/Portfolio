...
function StoryStep({ type, text }) {
  ...
  return (
    <div className="flex gap-2.5">
      ...
      <div>
        <span className={`text-[10px] font-mono uppercase tracking-wider ${STEP_COLORS[type]}`}>
          {STEP_LABELS[type]}
        </span>
...
export default function ProjectCard({ project }) {
  ...
  return (
    ...
    >
      ...
      {/* Insight */}
      {project.insight && (
        <div className="flex gap-2 items-start bg-accent/5 border border-accent/10 rounded-lg px-3 py-2.5 mb-3">
          <svg className="w-3.5 h-3.5 text-accent shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
